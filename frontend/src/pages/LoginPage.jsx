...
export default function LoginPage() {
  ...
  async function handleLogin(provider = 'google') {
    setError(null);
    setLoading(true);
    if (isDesktop) setWaitingForBrowser(true);
    try {
      ...
      setError(err.stack || err.message || String(err));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{
          ...
          {isDesktop
            ? 'Sign in via your browser to connect your Aether account to this desktop app.'
            : t('login_desc')}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', alignItems: 'center', width: '100%' }}>
        {isDesktop ? (
          <button
            ...
            disabled={loading || waitingForBrowser}
            style={{
              ...btnStyle,
          ...
          >
            ...
            {waitingForBrowser ? 'Waiting...' : 'Sign in to Aether'}
          </button>
        ) : (
          <>
            <button
      ...
      {waitingForBrowser && (
        <div style={{
          ...
          <p style={{ color: 'var(--primary)', fontSize: '0.9rem', fontWeight: 600, margin: '0 0 4px' }}>
