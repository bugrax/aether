...
import MobileVaultSheet from './MobileVaultSheet';
...
export default function Sidebar({ labels = [], onLabelsChanged }) {
  const { user, logout, isDevMode, isAdmin } = useAuth();
  ...
  const [mobileVaultSheetOpen, setMobileVaultSheetOpen] = useState(false);
  ...
  return (
    <>
      ...
      <header className="mobile-top-header">
        ...
        <button className="mobile-vault-button" onClick={() => setMobileVaultSheetOpen(true)}>
          {currentVault ? (
            <>
              <span className="mobile-vault-button-icon">{currentVault.icon || '🗂️'}</span>
              <span className="mobile-vault-button-name">{currentVault.name}</span>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="6 9 12 15 18 9"/>
              </svg>
            </>
          ) : (
            <span className="mobile-header-logo">Aether</span>
          )}
        </button>
      ...
      <MobileVaultSheet
        open={mobileVaultSheetOpen}
        onClose={() => setMobileVaultSheetOpen(false)}
      />
    </>
  );
}
